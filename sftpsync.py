...
def _validate_identity_file(path):
    if not os.path.exists(path):
        raise ValueError('Invalid path. "%s" does NOT exist. Please provide a valid path to identity file.' % path)
    return path

def _validate_and_parse_socks_version(socks_version, white_list=['SOCKS4', 'SOCKS5']):
...
def proxy_socket(host, port=1080, username=None, password=None, version=socks.SOCKS5):
    ...
    proxy_socket.set_proxy(proxy_type=version, addr=host, port=port, username=username, password=password)
    return proxy_socket
    ...
    try:
        ...
        for opt, value in opts:
            ...
            if opt in ('-i', '--identity'):
                identity_file = _validate_identity_file(value)
            if opt == '--proxy':
                proxy = _parse_connection_string(value)
            if opt == '--proxy-version':
                proxy_version = _validate_and_parse_socks_version(value)
        ...
        sftp = _validate_connection_details(connection_string)
        local_path = _validate_path(local_path)

        ssh_config = {}
        if identity_file:
            ssh_config.update(ssh_config(sftp[_HOST], identity_file))
            # Infer as much as possible using current user's SSH configuration:
            #config = ssh_config(host)

        if len(ssh_options) > 0:
            pass

        if proxy:
            proxy_socket = proxy_socket(proxy[_HOST], proxy[_PORT], proxy[_USER], proxy[_PASS], proxy_version)
